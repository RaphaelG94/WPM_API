...
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[User]') AND [c].[name] = N'UpdatedByUserId');
IF @var31 IS NOT NULL EXEC(N'ALTER TABLE [User] DROP CONSTRAINT [' + @var31 + '];');
...
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[User]') AND [c].[name] = N'DeletedByUserId');
IF @var32 IS NOT NULL EXEC(N'ALTER TABLE [User] DROP CONSTRAINT [' + @var32 + '];');
...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20180921104532_34_Many2ManyCategoryShopItem', N'2.1.3-rtm-32065');
...
IF @var33 IS NOT NULL EXEC(N'ALTER TABLE [Hardware] DROP CONSTRAINT [' + @var33 + '];');
...
IF @var34 IS NOT NULL EXEC(N'ALTER TABLE [Hardware] DROP CONSTRAINT [' + @var34 + '];');
...
IF @var35 IS NOT NULL EXEC(N'ALTER TABLE [Hardware] DROP CONSTRAINT [' + @var35 + '];');
